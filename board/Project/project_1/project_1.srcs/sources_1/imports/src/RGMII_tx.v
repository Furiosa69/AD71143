module RGMII_tx (
    ...
    output wire [2:0]   dbg_state   //调试: 当前状态机状态
);
    ...
    // TXD:   上升沿 = tx_byte[7:4] (高半字节), 下降沿 = tx_byte[3:0] (低半字节)
    //
    // 注意: YT8531C PHY的RGMII接收端将上升沿数据解析为高半字节[7:4],
    //       下降沿数据解析为低半字节[3:0], 与标准RGMII(IEEE 802.3 Clause 35)相反.
    //       因此D1(上升沿) = 高半字节, D2(下降沿) = 低半字节.
    ...
    ODDR #(
        ...
        .C (TXC),
        .CE(1'b1),
        .D1(tx_byte[4]),       // 上升沿 = bit4 (高半字节)
        .D2(tx_byte[0]),       // 下降沿 = bit0 (低半字节)
        .R (1'b0),
        .S (1'b0)
    );
    ...
    ODDR #(
        ...
        .C (TXC),
        .CE(1'b1),
        .D1(tx_byte[5]),       // 上升沿 = bit5 (高半字节)
        .D2(tx_byte[1]),       // 下降沿 = bit1 (低半字节)
        .R (1'b0),
        .S (1'b0)
    );
    ...
    ODDR #(
        ...
        .C (TXC),
        .CE(1'b1),
        .D1(tx_byte[6]),       // 上升沿 = bit6 (高半字节)
        .D2(tx_byte[2]),       // 下降沿 = bit2 (低半字节)
        .R (1'b0),
        .S (1'b0)
    );
    ...
    ODDR #(
        ...
        .C (TXC),
        .CE(1'b1),
        .D1(tx_byte[7]),       // 上升沿 = bit7 (高半字节)
        .D2(tx_byte[3]),       // 下降沿 = bit3 (低半字节)
        .R (1'b0),
        .S (1'b0)
    );
