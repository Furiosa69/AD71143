...
module ad71143_data_rx_dual #(
    ...
    //
    // 单 Panel 超时: 当只有一路 Panel 有数据时, 等待 CAP_TIMEOUT 周期后
    // 超时输出该路数据, 另一路补 0. 适用于仅连接一路 AD71143 的调试场景.
    // =========================================================================
    ...
    always @(posedge clk_sys or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (p0_captured && p1_captured) begin
                ...
                p1_captured        <= 1'b0;
                cap_timeout_cnt    <= 8'd0;
            end else if (p0_captured ^ p1_captured) begin
                // 只有一路有数据 → 启动超时计数器
                if (cap_timeout_cnt >= CAP_TIMEOUT - 1) begin
                    // 超时 → 输出有数据的那一路, 另一路补 0
                    if (p0_captured) begin
                        merged_burst       <= {128'd0, panel0_hold};
                        merged_burst_index <= p0_burst_index;
                    end else begin
                    ...
                    cap_timeout_cnt <= cap_timeout_cnt + 1'b1;
                end
            end else begin
                cap_timeout_cnt <= 8'd0;
            end
