...
module nt39565d_gate_ctrl #(
    ...
    // localparam [3:0] XAO_SHUT_S      = 4'd9;  // 已禁用
    localparam [3:0] WAIT_LINE       = 4'd11;  // 改成 11，跳过 9 和 10
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 状态保护：强制将状态限制在有效值范围内
            // 只捕获状态 9 和 10，允许 11 (WAIT_LINE)
            if (state == 4'd9 || state == 4'd10) begin
                state <= IDLE;
                cnt <= 16'd0;
                shift_cnt <= 16'd0;
                busy <= 1'b0;
                ...
                default: begin
                    // 捕获所有未定义状态（包括 XAO_SHUT_S=9），强制返回 IDLE
                    state      <= IDLE;
                    cnt        <= 16'd0;
                    shift_cnt  <= 16'd0;
                    busy       <= 1'b0;
