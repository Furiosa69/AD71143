...
module top #(
  ...
  localparam CFG_READ_ISSUE = 3'd4;
  localparam CFG_FINISHED   = 3'd5;
  localparam CFG_READ_WAIT  = 3'd6;
  ...
  assign cfg_data_lut =
      (cfg_reg_idx == 4'd0)  ? 10'h130 :  // Reg3:  AZEN=1, REFDAC=48 (REF_OUT=1.25V)
      ...
      (cfg_reg_idx == 4'd4)  ? 10'h082 :  // Reg4:  pipeline INTRST_C=8, INTRST_O=2
      (cfg_reg_idx == 4'd5)  ? 10'h014 :  // Reg5:  pipeline CDS1_C=1, CDS1_O=4
      (cfg_reg_idx == 4'd6)  ? 10'h057 :  // Reg6:  pipeline CDS2_C=5, CDS2_O=7
      (cfg_reg_idx == 4'd7)  ? 10'h036 :  // Reg7:  pipeline FA_CDS1=3, FA_CDS2=6
      (cfg_reg_idx == 4'd8)  ? 10'h000 :  // Reg8:  custom clamp disabled
      (cfg_reg_idx == 4'd9)  ? 10'h007 :  // Reg9:  preserve reserved reset value
      (cfg_reg_idx == 4'd10) ? 10'h000 :  // Reg10: pipeline averaging disabled
      (cfg_reg_idx == 4'd11) ? 10'h018 :  // Reg11: LFSR disabled; preserve reserved bits
      (cfg_reg_idx == 4'd12) ? 10'h002 :  // Reg12: low power disabled; preserve reserved bits
      (cfg_reg_idx == 4'd13) ? 10'h023 :  // Reg13: preserve reserved reset value
      (cfg_reg_idx == 4'd14) ? 10'h02B :  // Reg14: preserve reserved reset value
      (cfg_reg_idx == 4'd15) ? 10'h008 :  // Reg15: preserve reserved reset value
      10'h000;
      ...
      case (cfg_state)
          ...
          CFG_DONE: begin
              if (cfg_reg_idx != 4'd15 && cfg_gap_done)
                  ...
                  cfg_state_next = CFG_READ_ISSUE;
          end
          CFG_READ_ISSUE: cfg_state_next = CFG_READ_WAIT;
          CFG_READ_WAIT:  if (spi_done) cfg_state_next = CFG_FINISHED;
          ...
          case (cfg_state)
              ...
              CFG_DONE: begin
                  if (cfg_gap_done) begin
                      cfg_gap_cnt <= 7'd0;
                      if (cfg_reg_idx == 4'd15) begin
                          // Read Reg3 to verify AZEN and the REF_OUT DAC code.
                          cfg_reg_idx <= 4'd0;
                          cfg_spi_rw  <= 1'b0;
                      end else
  ...
  assign spi_reg_data = cfg_spi_rw ? cfg_data_lut : 10'h000;
  ...
  nt39565d_gate_ctrl #(
      .CLK_FREQ_MHZ      (50  ),
      .CPV_PERIOD_US     (62  ),  // 30fps: 62μs周期 (CPV=16.13kHz, 实际29.8fps)
      .CPV_PW_US_X10     (310 ),  // 31μs脉冲宽度 (50% duty cycle)
      .STV_SETUP_US_X10  (3   ),
      .STV_HOLD_US_X10   (4   ),
      .OE_PW_US_X10      (450 ),  // 45μs (约73% duty，保持足够的Gate输出窗口)
      .XAO_DELAY_US      (20  ),
      .OUT_CH_NORMAL     (541 ),
      .OUT_CH_2G         (540 ),
      .DEFAULT_MODE_SEL  (0   ),
...
ila_1 u_ila_datapath (
    ...
    .probe1 (cfg_state),
    // probe2 is the complete Reg3 readback; expected value is 10'h130.
    .probe2 (spi_rdback_p1),
