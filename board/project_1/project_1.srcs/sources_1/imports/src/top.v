...
module top #(
    ...
    parameter integer FRAME_GAP_CYCLES = 250000,  // 修复: 5ms @ 50MHz (原100太短)
  ...
  // =========================================================================
  // IDELAYCTRL: IDELAYE2参考时钟控制器（200MHz）
  // 必须使用200MHz时钟作为REFCLK，用于IDELAYE2的延迟校准
  // =========================================================================
  wire clk_200m;

  // 临时方案：使用clk_100m的2倍频或独立生成200MHz
  // 如果MMCM有多余输出，可以配置为200MHz；否则用BUFG + PLL
  // 这里假设用clk_100m暂代（需要后续修改MMCM配置输出真正的200MHz）
  ...
  assign cfg_data_lut =
      ...
      (cfg_reg_idx == 4'd1)  ? 10'h003 :  // Reg0:  PWR=Normal(000), IFS=3 (修复: 5→3, 降低增益避免饱和)
  ...
  ad71143_ctrl #(
      ...
      .LINE_CYCLES      (6000),     // 恢复正常�?60μs @ 100MHz
  ...
  // AD71143 �????? Panel LVDS 数据接收 (200MHz �?????)
  	ad71143_data_rx_dual #(
  	    ...
  	    .clk_sys              (clk_100m),
  	    .rst_n                (rst_n_100m),
  	    .sync_in              (line_start_pulse),  // 修改：使用 line_start_pulse 而不是 sync_int
  	    .aclk_done            (aclk_done),
  	    .dclk_p_A0            (dclk_p_A0),
  	    .dclk_n_A0            (dclk_n_A0),
  ...
  // 关键修复：
  // 1. CPV_PERIOD_US: 10→6 (手册注释：实际支持最小2μs，用6μs保守)
  // 2. CPV_PW_US_X10: 10→30 (3μs, 50% duty cycle)
  // 3. OE_PW_US_X10: 10→45 (4.5μs, 75% duty扩大Gate输出窗口)
  nt39565d_gate_ctrl #(
      .CLK_FREQ_MHZ      (50  ),
      .CPV_PERIOD_US     (6   ),  // 修复: 6μs (Gout=12μs，手册实测可到2μs)
  ...
  // RGMII Bridge - 禁用测试模式，只输出真实 AFE 数据
  // TEST_MODE = 0: 禁用测试数据生成
  // 如果 FIFO 为空，将不发送数据包（�?�不是发送测试数据）
  rgmii_bridge #(
      .TEST_MODE     (0),           // 禁用测试模式
      .TEST_INTERVAL (10000)        // 保留参数（但不会使用�?
  ) u_rgmii_bridge (
      .rst_n      (pll_locked  ),
      .clk_100m   (clk_100m    ),
      .clk_125m   (clk_125m    ),
...
// =========================================================================
// ILA 调试探针 2 (Panel 1 AD71143 数据接收诊断, clk_100m �????)
// �???? Vivado IP Catalog 中新�???? ILA, Component Name = ila_0_1
...
// =========================================================================
// ILA 调试探针 3 (NT39565D Gate Driver 诊断, gate_clk/50MHz �???)
// �??? Vivado IP Catalog 中新�??? ILA, Component Name = ila_nt39
// 监测 NT39 模块的关键信号：状�?�机、计数器、控制信�???
// =========================================================================
//ila_0 u_ila_nt39 (
//    .clk    (gate_clk),
//    // probe0 [7:0]: 控制信号 {cpv, stv1, stv2, oe1, busy, line_done, frame_done, frame_start}
//    .probe0 ({cpv, stv1, stv2, oe1, busy, line_done, frame_done, frame_start}),
//    // probe1 [3:0]: FSM 状�??
//    .probe1 (nt39_state),
//    // probe2 [15:0]: 时序计数�???
//    .probe2 (nt39_cnt),
//    // probe3 [15:0]: 行扫描计数器
//    .probe3 (nt39_shift_cnt),
//    // probe4 [15:0]: 目标行数
//    .probe4 (nt39_target_lines),
//    // probe5 [7:0]: 配置和输出信�??? {ud, lr, mode1, mode2, sel, stv_mode, chip_sel1, chip_sel2}
//    .probe5 ({ud, lr, mode1, mode2, sel, stv_mode, chip_sel1, chip_sel2}),
//    // probe6 [0:0]: line_start_pulse 触发信号
//    .probe6 (line_start_pulse)
//);

// =========================================================================
// ILA 调试探针 5 (数据通路�?? SPI 诊断, clk_100m �??)
// �?? Vivado IP Catalog 中新�?? ILA, Component Name = ila_datapath
// 监测数据是否�?? AFE 正确传输�?? RGMII，以�?? SPI 配置状�??
// =========================================================================
ila_1 u_ila_datapath (
    ...
    .probe0 ({
        ...
        init_done,             // [6] AD71143 初始化完�??
        aclk_done,             // [5] ACLK 序列完成
        line_start,            // [4] 行开始信�??
        line_done,             // [3] 行完成信�??
        dbg_p0_merged_valid,   // [2] Panel 0 数据有效
        dbg_p1_merged_valid,   // [1] Panel 1 数据有效
        merged_valid           // [0] 合并后数据有�??
    }),

    // probe1 [1:0]: SPI 配置 FSM 状�??
    .probe1 (cfg_state),

    // probe2 [3:0]: SPI 寄存器索�??
    .probe2 (cfg_reg_idx),

    // probe3 [2:0]: Panel 1 状�?�机状�??
    .probe3 (dbg_p1_state),

    // probe4 [6:0]: Panel 1 burst 索引
    .probe4 (dbg_p1_burst_index),

    // probe5 [7:0]: Panel 1 调试信号
    .probe5 ({
        dbg_p1_shift_hi[7:0]
    }),

    // probe6 [7:0]: Panel 1 调试信号
    .probe6 ({
        dbg_p1_shift_lo[7:0]
    }),

    // probe7 [6:0]: 其他状态 + ACLK + LVDS原始输入
    .probe7 ({
        dbg_p1_dout_b_raw,     // [6] LVDS B原始输入
        dbg_p1_dout_a_raw,     // [5] LVDS A原始输入
        aclk_int,              // [4] ACLK信号
        dbg_p1_burst_en,       // [3] Panel 1 burst 使能
        dbg_p1_header_ok,      // [2] Panel 1 header 正确
        dbg_p1_line_done,      // [1] Panel 1 行完成
        cfg_spi_start          // [0] SPI 启动信号
    })
);
