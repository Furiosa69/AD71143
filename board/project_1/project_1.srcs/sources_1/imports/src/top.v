...
module top #(
  ...
  localparam CFG_READBACK = 3'd4;  // 新增：读回Reg3验证
  ...
  assign cfg_data_lut =
      ...
      (cfg_reg_idx == 4'd11) ? 10'h001 :  // Reg11: LFSR_EN=0 (禁用LFSR测试模式，输出真实ADC数据)
      ...
      case (cfg_state)
          ...
          CFG_DONE: begin
              if (cfg_reg_idx != 4'd15 && cfg_gap_done)
                  ...
                  cfg_state_next = CFG_READBACK;  // 写完�?有寄存器后读回Reg3
              // else stay in CFG_DONE permanently (all 16 regs written)
          end
          CFG_READBACK: begin
              // 保持在读回状态，持续读取Reg3
              cfg_state_next = CFG_READBACK;
          end
  ...
  always @(posedge clk_100m or negedge rst_n_100m) begin
      if (!rst_n_100m) begin
          ...
          case (cfg_state)
              ...
              CFG_READBACK: begin
                  // 持续读取Reg3: cfg_reg_idx=0对应地址3
                  cfg_reg_idx <= 4'd3;      // cfg_reg_idx=0 �? cfg_addr_lut=3 (Reg3)
                  cfg_spi_rw  <= 1'b0;      // 读模�?
                  if (cfg_gap_done) begin
                      cfg_gap_cnt <= 7'd0;
                      cfg_spi_start <= 1'b1;
  ...
  assign spi_cs_active = (cfg_state == CFG_READBACK) || !cfg_all_done;
