// =============================================================================
// AD71143 �? Panel LVDS 数据接收 �? DDR 模式 (DOUTMODE=1)
// =============================================================================
// 封装两个 ad71143_data_rx 实例, 共享 sync_in/aclk_done, 各自独立 LVDS 物理�?
// �? Panel: 2 �? DOUT (Lane A/B) + 1 �? DCLK �? DDR 模式
// �? Panel 共计: 4 �? DOUT + 2 �? DCLK (内部回环采样, �? DCLKO 引脚)
//
...
module ad71143_data_rx_dual #(
    ...
    // 合并输出 (clk_sys �?)
    // =========================================================================
    output wire         line_done,           // �? Panel 均完成一�?
    output wire         header_ok,           // �? Panel Header �? 0x0A
    output reg          merged_valid = 1'b0, // 合并数据有效 (单周期脉�?)
    ...
    // 调试
    // =========================================================================
    output wire [2:0]   state_debug_0,      // Panel 0 状�?�机
    output wire [2:0]   state_debug_1,      // Panel 1 状�?�机
    output wire         burst_en_out,       // Panel 0 burst_en (TB 同步�?)
    ...
    // 输出对齐逻辑 + �? Panel 超时降级
    //
    // 【测试模式�?�：只输�? Panel 1 数据，不等待 Panel 0
    // Panel 0 数据位置�? 0，简化调�?
    // =========================================================================
    localparam integer CAP_TIMEOUT = 20;  // �? Panel 超时周期�?
    ...
    always @(posedge clk_sys or negedge rst_n) begin
        if (!rst_n) begin
            ...
            merged_valid <= 1'b0;  // 默认�?, 仅对齐成功时发单周期脉冲

            // 【测试模式�?�：忽略 Panel 0，只处理 Panel 1
            // 捕获 Panel 0 数据（保留�?�辑但不使用�?
            ...
            // 【测试模式�?�：只要 Panel 1 数据到达，立即输�?
            // Panel 0 位置填充 0
            if (p1_captured) begin
                merged_burst       <= {panel1_hold, 128'd0};  // Panel 1 数据放在�? 128 �?
                merged_burst_index <= p1_burst_index;
                merged_valid       <= 1'b1;
                p0_captured        <= 1'b0;
                p1_captured        <= 1'b0;
    ...
    // =========================================================================
    // 行级状�??: �? Panel 均完成才有效 (�? Panel 模式取有数据的一�?)
    // =========================================================================
    ...
    assign burst_en_out  = p0_burst_en;   // Panel 0 �? burst_en, TB �?
